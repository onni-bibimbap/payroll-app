...
def recompute(slip: Payslip, cfg: statutory.RateConfig = statutory.DEFAULT_CONFIG,
              lindung_rate: Decimal = Decimal("0")) -> None:
...
def _new_payslip(emp: Employee, run: PayrollRun, cfg: statutory.RateConfig, settings=None) -> Payslip:
    ...
    slip = Payslip(
        ...
        include_allowance=True,    # statutory base = basic + allowance, always
        include_ot=False,          # OT is never part of the statutory base
